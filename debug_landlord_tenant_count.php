...
$properties = App\Models\Property::where('owner_id', $landlord->id)->get();
...
foreach ($properties as $property) {
    ...
    $propertyWithNew = App\Models\Property::where('id', $property->id)
        ->withCount(['units'])
...
$propertiesWithCounts = App\Models\Property::where('owner_id', $landlord->id)
    ->withCount(['units'])
    ->with(['tenancies' => function ($query) {
        $query->where('tenancies.status', 'active');
    }])
