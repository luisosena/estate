...
export default function NotificationBell({ initialUnreadCount = 0 }: NotificationBellProps) {
  ...
  const fetchUnreadCount = async () => {
    try {
      const response = await fetch(route('landlord.notifications.unread-count'));
  ...
  const fetchRecentNotifications = async () => {
    try {
      const response = await fetch(route('landlord.notifications.recent'));
      const data = await response.json();
  ...
  const markAsRead = async (id: string) => {
    try {
      await router.put(route('landlord.notifications.read', id), {}, {
        preserveScroll: true,
        onSuccess: () => {
          // Update local state
  ...
  const markAllAsRead = async () => {
    try {
      await router.put(route('landlord.notifications.read-all'), {}, {
        preserveScroll: true,
        onSuccess: () => {
          // Update local state
          setNotifications(prev => 
